...
%type<intNumber> pick_PLUS_MINUS pick_multop pick_unop augassign
%type<node> expr_stmt del_stmt pass_stmt import_stmt global_stmt exec_stmt assert_stmt break_stmt continue_stmt return_stmt raise_stmt yield_stmt import_name import_from
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist
		{
			if($2 == PLUSEQUAL){
				$$ = new AsgPlusBinaryNode($1, $3);
				pool.add($$);
			}else if($2 == MINEQUAL){
				$$ = new AsgMinusBinaryNode($1, $3);
				pool.add($$);
			}else if($2 == STAREQUAL){
				$$ = new AsgMultBinaryNode($1, $3);
				pool.add($$);
			}else if($2 == SLASHEQUAL){
				$$ =  new AsgDivBinaryNode($1, $3);
				pool.add($$);
			}else if($2 == PERCENTEQUAL){
				$$ = new AsgModBinaryNode($1, $3);
				pool.add($$);
			}else if($2 == DOUBLESTAREQUAL){
				$$ = new AsgDoubleStarBinaryNode($1, $3);
				pool.add($$);
			}else if($2 == DOUBLESLASHEQUAL){
				$$ = new AsgDdivBinaryNode($1, $3);
				pool.add($$);
			}
		}
	| testlist star_EQUAL
