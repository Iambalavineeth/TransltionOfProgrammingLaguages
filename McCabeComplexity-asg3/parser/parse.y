...
%type<number> suite stmt plus_stmt compound_stmt if_stmt while_stmt for_stmt try_stmt decorated
%type<number> star_ELIF plus_except opt_FINALLY classdef funcdef with_stmt opt_testlist
%type<number> opt_ELSE star_NEWLINE_stmt except_clause pick_NEWLINE_stmt decorators decorator
%type<number> simple_stmt small_stmt star_SEMI_small_stmt star_COMMA_with_item opt_IF_ELSE testlist
%type<number> star_COMMA_test or_test test dotted_name opt_arglist arglist lambdef
%token<id> NAME
...
%token LESSEQUAL LPAR LSQB MINEQUAL MINUS NOT NOTEQUAL NUMBER
...
funcdef // Used in: decorated, compound_stmt
	...
	{
		$$ = 1 + $5;
		//Function present in the first column
		if(@1.first_column - 1 == 0){
			std::cout << "(\"" << @1.first_line << ":";
			std::cout << @1.first_column-1 << ": \'" << $2;
			std::cout << "\'\", " << $$ << ")" << std::endl;
		}
		if(!class_name.empty()){
			std::cout << "(\"" << @1.first_line << ":";
			std::cout << @1.first_column-1 << ": \'" << class_name << "." << $2;  				std::cout << "\'\", " << $$ << ")" << std::endl; 			
		}
		delete[] $2;
		//Iterate through all the classes - TODO		
	}
	;
...
try_stmt // Used in: compound_stmt
	...
	| TRY COLON suite FINALLY COLON suite { $$ = 0; }
	;
...
classdef // Used in: decorated, compound_stmt
	: CLASS NAME {class_name = (class_name.length() > 0)?(class_name+"."+$2):$2;} LPAR opt_testlist RPAR COLON suite {class_name.clear(); delete[] $2;}
	| CLASS NAME {class_name = (class_name.length() > 0)?(class_name+"."+$2):$2;} COLON suite {class_name.clear(); delete[] $2;}
	;
...
arglist // Used in: opt_arglist
	: star_argument_COMMA pick_argument { $$ = 0; }
	;
